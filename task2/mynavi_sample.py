...
#log_file_path = LOG_FILE_PATH.format(date=datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S'))

def set_driver(driver_path,headles_flg):
...
def main():
    ...
    driver = set_driver("C:\\Users\Kanomata\Desktop\chromedriver.exe", 'false')
    ...
    while True:
        ...
        fee_list = driver.find_elements_by_css_selector(".cassetteRecruit .tableCondition")
    
        
        for company_name, sell_point, employee_status, fee in zip(company_name_list,sell_point_list,employee_status_list,fee_list):        
            try:
